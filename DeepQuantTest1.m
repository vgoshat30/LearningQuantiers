...
v_fRate =  2; %1:0.2:3;%[1,2];   % Quantization rate
...
v_nCurves   = [...          % Curves
    ...
    1 ...                   % DNN, Soft-to-hard, CSI uncertainty
    1 ...                   % DNN, Passing gradient, optimal training  
    1 ...                   % DNN, Passing gradient, CSI uncertainty
...
for ii=1:length(v_fSNRdB)
    ...
    for jj=1:length(v_fRate)
        ...
        if (v_nCurves(3) == 1)                  
            % Get network
            v_cNet = GetQuantNet(m_fYtrain', v_fDtrain', s_nP, codewordsNum);     
       ...
       % Deep task-based quantizer passing gradient
        ...
        if (v_nCurves(7) == 1)
            ...
            m_fBER(7,s_fXidx,s_fYidx) = s_fMAP(m_fQuant(m_fYtest, s_nLevels, s_fDynRange),...
                                               m_fStest, m_fH, sqrt(s_fSigW)*eye(s_nN));
        end
